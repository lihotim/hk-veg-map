...
NOTICE_TEXT = '''
    更新至2024年11月27日。「素食類型」如果查不到資料一律當「蛋奶素」，請自行向店家查詢。大部分資料來自Google及OpenRice，如有任何錯漏敬請見諒。
'''
...
USER_GUIDE = '''
    ...
    2. 點選【地區】，用法同上。
    3. 表格會顯示合乎選擇條件的餐廳名單。
    4. 素食地圖會顯示所有合乎選擇條件餐廳的座標（綠色氣球）。點擊座標可以顯示該餐廳的詳細資料，以及是OpenRice連結。
    5. 由於Google提供的座標未必完全準確，座標位置跟餐廳的實際位置可能會有出入。請自行用地址確認餐廳位置。祝您用餐愉快！
'''
